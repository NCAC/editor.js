...
import { Dom } from '../../dom';
import { API, BlockTool, BlockToolConstructorOptions, BlockToolData, HTMLPasteEvent  } from '../../../../types';
import ParagraphSvgIcon from './toolbox-icon.svg'
 ...
 * @property {number[]} levels — Paragraph levels
 * @property {number} defaultType — default type
 */
 ...
 * @property {number} type - Paragraph's type
 */
...
export class Paragraph implements BlockTool {
  ...
  private _settings: ParagraphConfig;
   ...
   * @private
   */
  normalizeData(data: { text?: string, level?: string | number}): ParagraphData {
    const newData: ParagraphData = {};
    if (typeof data !== 'object') {
      data = {};
    }
    newData.text = data.text || '';
    newData.level = parseInt((data.level as string), 10) || this.defaultLevel.number;
