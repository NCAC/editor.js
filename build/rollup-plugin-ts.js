...
function finalizeParsedCommandLine({
  cwd,
...
function shouldDebugSourceFile(debug, {
  fileName,
  text
}) {
...
function shouldDebugMetrics(debug, sourceFile) {
  if (typeof debug === "boolean")
    return debug;
  return Boolean(debug(Object.assign({
    kind: "metrics"
  }, (sourceFile == null ? {} : {
...
function getParsedCommandLine(options) {
  ...
  // If the user provided JSON-serializable ("raw") CompilerOptions directly, use those to build a ParsedCommandLine
    ...
    }, fileSystem, cwd).options;
    parsedCommandLine = typescript.parseJsonConfigFileContent({
      compilerOptions: tsconfig
    }, fileSystem, cwd, forcedCompilerOptions);
  }
  ...
  finalizeParsedCommandLine(Object.assign(Object.assign({}, options), {
    parsedCommandLineResult
  }));
 ...
 */
function getForcedModuleKindOption({
  pluginOptions
}) {
  ...
  return {
    module: pluginOptions.typescript.ModuleKind.ESNext
  };
}
/**
 * Gets the ScriptTarget to force
 */
function getForcedScriptTargetOption({
  pluginOptions,
  browserslist
}) {
  ...
  if (pluginOptions.transpiler === "babel") {
    return {
      target: pluginOptions.typescript.ScriptTarget.ESNext
    };
  }
  // If a Browserslist is provided, and if Typescript should perform the transpilation, decide the appropriate ECMAScript version based on the Browserslist.
    ...
    return {
      target: getScriptTargetFromBrowserslist(browserslist, pluginOptions.typescript)
    };
  }
...
function getForcedCompilerOptions(options) {
  return Object.assign(Object.assign(Object.assign({}, getForcedModuleKindOption(options)), getForcedScriptTargetOption(options)), {
  ...
  diagnostics.forEach((diagnostic) => {
    ...
    switch (diagnostic.category) {
      case typescript.DiagnosticCategory.Error:
        context.error(Object.assign(Object.assign(Object.assign(Object.assign({
          frame,
          code,
          name: code,
          stack
          ...
          loc: Object.assign(Object.assign(Object.assign({}, (diagnostic.file == null ? {} : {
      ...
      case typescript.DiagnosticCategory.Suggestion:
        context.warn(Object.assign(Object.assign(Object.assign(Object.assign({
          frame,
          code,
          name: code
        }, (diagnostic.length == null ? {} : {
...
function getRealFileSystem(typescript) {
  return Object.assign(Object.assign({}, typescript.sys), {
    realpath(path) {
...
function getPluginOptions(options) {
  ...
  if (transpiler === "typescript") {
    ...
    return Object.assign(Object.assign(Object.assign({}, baseOptions), ("babelConfig" in options ? {
...
function combineConfigItems(userItems, defaultItems = [], forcedItems = [], inChunkPhase) {
  const namesInUserItems = new Set(userItems.map(item => {
    var _a;
    return (_a = item.file) === null || _a === void 0 ? void 0 : _a.resolved;
  }).filter(isDefined));
  const namesInForcedItems = new Set(forcedItems.map(item => {
    var _a;
    return (_a = item.file) === null || _a === void 0 ? void 0 : _a.resolved;
  }).filter(isDefined));
...
function configItemIsRelevantForChunkPhase(configItem) {
  return (BABEL_MINIFY_PRESET_NAMES.some(preset => {
    var _a;
    return (_a = configItem.file) === null || _a === void 0 ? void 0 : _a.resolved.includes(preset);
  }) || BABEL_MINIFY_PLUGIN_NAMES.some(plugin => {
    var _a;
    return (_a = configItem.file) === null || _a === void 0 ? void 0 : _a.resolved.includes(plugin);
  }));
}
/**
...
function getBabelConfig({
  ...
  return (filename) => {
    ...
    const partialConfig = core.loadPartialConfig(
      ...
      isBabelInputOptions(babelConfig) ? // If the given babelConfig is an object of input options, use that as the basis for the full config
      Object.assign({
        cwd,
        root: cwd
      }, babelConfig) : // Load the path to a babel config provided to the plugin if any, otherwise try to resolve it
      Object.assign({
        cwd,
        root: cwd,
        filename
    ...
    } = forcedOptions, otherForcedOptions = tslib.__rest(forcedOptions, ["presets", "plugins"]);
    const {
      presets: defaultPresets,
      plugins: defaultPlugins
    } = defaultOptions, otherDefaultOptions = tslib.__rest(defaultOptions, ["presets", "plugins"]);
    ...
    const combined = Object.assign(Object.assign(Object.assign(Object.assign({}, otherDefaultOptions), options), otherForcedOptions), {
      presets: combineConfigItems(((_a = options.presets) !== null && _a !== void 0 ? _a : []), defaultPresets == null ? undefined : (_c = (_b = core.loadPartialConfig(Object.assign({
        presets: defaultPresets
      }, configFileOption))) === null || _b === void 0 ? void 0 : _b.options.presets) !== null && _c !== void 0 ? _c : undefined, forcedPresets == null ? undefined : (_e = (_d = core.loadPartialConfig(Object.assign({
        presets: forcedPresets
      }, configFileOption))) === null || _d === void 0 ? void 0 : _d.options.presets) !== null && _e !== void 0 ? _e : undefined, phase === "chunk"),
      plugins: combineConfigItems(((_f = options.plugins) !== null && _f !== void 0 ? _f : []), defaultPlugins == null ? undefined : (_h = (_g = core.loadPartialConfig(Object.assign({
        plugins: defaultPlugins
      }, configFileOption))) === null || _g === void 0 ? void 0 : _g.options.plugins) !== null && _h !== void 0 ? _h : undefined, forcedPlugins == null ? undefined : (_k = (_j = core.loadPartialConfig(Object.assign({
        plugins: forcedPlugins
      }, configFileOption))) === null || _j === void 0 ? void 0 : _j.options.plugins) !== null && _k !== void 0 ? _k : undefined, phase === "chunk")
    });
    ...
    const loadedOptions = (_o = core.loadOptions(Object.assign(Object.assign(Object.assign({}, combinedOptionsAfterHook), {
      filename
    }), configFileOption))) !== null && _o !== void 0 ? _o : undefined;
 ...
 */
function getForcedBabelOptions({
  cwd
}) {
  return {
...
class ResolveCache {
  ...
  get(options) {
    ...
    const {
      resolvedModule
    } = this.resolveModuleName(typescript, id, parent, compilerOptions, moduleResolutionHost);
  ...
  return {
    ...
    plugins: [
      ...
      [
        ...
        Object.assign(Object.assign({}, FORCED_BABEL_PLUGIN_TRANSFORM_RUNTIME_OPTIONS), {
          corejs: false
        })
      ]
    ]
...
function createExportSpecifierFromNameAndModifiers({
  name,
  modifiers,
  typescript,
  compatFactory
...
function visitModuleDeclaration(_a) {
  var {
    node
  } = _a, options = tslib.__rest(_a, ["node"]);
  if (node.body == null)
    return;
...
function visitNode(_a) {
  ...
  if (options.typescript.isImportDeclaration(node)) {
    return visitImportDeclaration(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (options.typescript.isImportTypeNode(node)) {
    return visitImportTypeNode(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (options.typescript.isModuleDeclaration(node)) {
    return visitModuleDeclaration(Object.assign(Object.assign({}, options), {
      node
    }));
...
function trackImportsTransformer(options) {
  ...
  const visitorOptions = Object.assign(Object.assign({}, options), {
    ...
    childContinuation: (node) => typescript.forEachChild(node, nextNode => {
      visitNode(Object.assign(Object.assign({}, visitorOptions), {
        node: nextNode
      }));
    }),
    continuation: (node) => {
...
function visitClassDeclaration(_a) {
  var {
    ...
    typescript,
    markAsExported
  } = _a, options = tslib.__rest(_a, ["node", "typescript", "markAsExported"]);
  // If the node has no export modifier, leave it as it is
  if (!hasExportModifier(node, typescript) || node.name == null)
    return;
  const {
    exportedSymbol
  } = createExportSpecifierFromNameAndModifiers(Object.assign(Object.assign({}, options), {
    name: node.name.text,
 ...
 */
function traceIdentifiersForBindingElement({
  node,
  continuation
}) {
 ...
 */
function traceIdentifiersForClassDeclaration({
  node,
  addIdentifier
}) {
  if (node.name == null)
 ...
 */
function traceIdentifiersForEnumDeclaration({
  node,
  addIdentifier
}) {
  addIdentifier(node.name.text);
 ...
 */
function traceIdentifiersForFunctionDeclaration({
  node,
  addIdentifier
}) {
  if (node.name == null)
 ...
 */
function traceIdentifiersForImportClause({
  node,
  addIdentifier,
  continuation
}) {
 ...
 */
function traceIdentifiersForNamespaceImport({
  node,
  addIdentifier
}) {
  if (node.name != null) {
 ...
 */
function traceIdentifiersForImportSpecifier({
  node,
  addIdentifier
}) {
  addIdentifier(node.name.text);
 ...
 */
function traceIdentifiersForIdentifier({
  node,
  addIdentifier
}) {
 ...
 */
function traceIdentifiersForInterfaceDeclaration({
  node,
  addIdentifier
}) {
  addIdentifier(node.name.text);
 ...
 */
function traceIdentifiersForTypeAliasDeclaration({
  node,
  addIdentifier
}) {
  if (node.name == null)
 ...
 */
function traceIdentifiersForVariableDeclaration({
  node,
  continuation
}) {
 ...
 */
function traceIdentifiersForExportAssignment({
  node,
  continuation
}) {
...
function traceIdentifiersForNode(_a) {
  ...
  if (options.typescript.isBindingElement(node))
    traceIdentifiersForBindingElement(Object.assign(Object.assign({}, options), {
      node
    }));
  else if (options.typescript.isClassDeclaration(node))
    traceIdentifiersForClassDeclaration(Object.assign(Object.assign({}, options), {
      node
    }));
  else if (options.typescript.isEnumDeclaration(node))
    traceIdentifiersForEnumDeclaration(Object.assign(Object.assign({}, options), {
      node
    }));
  else if (options.typescript.isFunctionDeclaration(node))
    traceIdentifiersForFunctionDeclaration(Object.assign(Object.assign({}, options), {
      node
    }));
  else if (options.typescript.isImportClause(node))
    traceIdentifiersForImportClause(Object.assign(Object.assign({}, options), {
      node
    }));
  else if (options.typescript.isNamespaceImport(node))
    traceIdentifiersForNamespaceImport(Object.assign(Object.assign({}, options), {
      node
    }));
  else if (options.typescript.isImportSpecifier(node))
    traceIdentifiersForImportSpecifier(Object.assign(Object.assign({}, options), {
      node
    }));
  else if (options.typescript.isExportSpecifier(node))
    traceIdentifiersForExportSpecifier(Object.assign(Object.assign({}, options), {
      node
    }));
  else if (options.typescript.isIdentifier(node))
    traceIdentifiersForIdentifier(Object.assign(Object.assign({}, options), {
      node
    }));
  else if (options.typescript.isInterfaceDeclaration(node))
    traceIdentifiersForInterfaceDeclaration(Object.assign(Object.assign({}, options), {
      node
    }));
  else if (options.typescript.isTypeAliasDeclaration(node))
    traceIdentifiersForTypeAliasDeclaration(Object.assign(Object.assign({}, options), {
      node
    }));
  else if (options.typescript.isVariableDeclaration(node))
    traceIdentifiersForVariableDeclaration(Object.assign(Object.assign({}, options), {
      node
    }));
  else if (options.typescript.isExportAssignment(node))
    traceIdentifiersForExportAssignment(Object.assign(Object.assign({}, options), {
      node
    }));
  else if (options.typescript.isCallExpression(node))
    traceIdentifiersForCallExpression(Object.assign(Object.assign({}, options), {
      node
    }));
  else if (options.typescript.isNewExpression(node))
    traceIdentifiersForNewExpression(Object.assign(Object.assign({}, options), {
      node
    }));
  else
...
function traceIdentifiers(_a, identifiers = new Set()) {
  var {
    node
  } = _a, rest = tslib.__rest(_a, ["node"]);
  return getIdentifiers(Object.assign(Object.assign({}, rest), {
    node
  }), identifiers);
}
...
function getIdentifiers(_a, identifiers) {
  ...
  traceIdentifiersForNode(Object.assign(Object.assign({}, rest), {
    node,
    continuation: nextNode => getIdentifiers(Object.assign(Object.assign({}, rest), {
      node: nextNode
    }), identifiers),
    childContinuation: nextNode => rest.typescript.forEachChild(nextNode, nextNextNode => {
      getIdentifiers(Object.assign(Object.assign({}, rest), {
        node: nextNextNode
      }), identifiers);
    }),
    addIdentifier(name) {
...
function visitVariableStatement(_a) {
  var {
    ...
    sourceFile,
    markAsExported
  } = _a, options = tslib.__rest(_a, ["node", "typescript", "sourceFile", "markAsExported"]);
  // If the node has no export modifier, leave it as it is
  if (!hasExportModifier(node, typescript))
    return;
  const identifiers = traceIdentifiers({
    node,
    sourceFile,
    typescript
  });
...
function visitNode$1(_a) {
  ...
  if (options.typescript.isClassDeclaration(node)) {
    return visitClassDeclaration(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (options.typescript.isClassExpression(node)) {
    return visitClassExpression(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (options.typescript.isFunctionDeclaration(node)) {
    return visitFunctionDeclaration(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (options.typescript.isFunctionExpression(node)) {
    return visitFunctionExpression(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (options.typescript.isEnumDeclaration(node)) {
    return visitEnumDeclaration(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (options.typescript.isInterfaceDeclaration(node)) {
    return visitInterfaceDeclaration(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (options.typescript.isTypeAliasDeclaration(node)) {
    return visitTypeAliasDeclaration(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (options.typescript.isModuleDeclaration(node)) {
    return visitModuleDeclaration$1(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (options.typescript.isVariableStatement(node)) {
    return visitVariableStatement(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (options.typescript.isExportDeclaration(node)) {
    return visitExportDeclaration(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (options.typescript.isExportAssignment(node)) {
    return visitExportAssignment(Object.assign(Object.assign({}, options), {
      node
    }));
  }
}
...
function trackExportsTransformer(options) {
  ...
  const visitorOptions = Object.assign(Object.assign({}, options), {
    ...
    childContinuation: (node) => typescript.forEachChild(node, nextNode => {
      visitNode$1(Object.assign(Object.assign({}, visitorOptions), {
        node: nextNode
      }));
    }),
    continuation: (node) => {
...
function statsCollector(options) {
  ...
  for (const importedSymbol of importedSymbols) {
    ...
    if (resolved == null)
      continue;
    resolveResults.push(Object.assign(Object.assign({}, resolved), {
  ...
  for (const resolveResult of resolveResults) {
    if (resolveResult.isExternalLibraryImport === true && resolveResult.packageId != null) {
      ...
      if (stats.externalTypes.some(({
          library
        }) => {
          var _a;
          return library === ((_a = resolveResult.packageId) === null || _a === void 0 ? void 0 : _a.name);
        }))
        continue;
...
function sourceFileBundler(options, ...transformers) {
  return context => bundle => {
    ...
    if (firstEntrySourceFile != null) {
      ...
      if (options.declarationStats != null) {
        Object.assign(options.declarationStats, {
          [options.declarationPaths.fileName]: statsCollector(Object.assign(Object.assign({}, visitorOptions), {
    ...
    if (bundleWithSyntheticLibReferences.syntheticLibReferences != null) {
      for (const {
          fileName
        } of bundleWithSyntheticLibReferences.syntheticLibReferences) {
    ...
    for (const updatedSourceFile of updatedSourceFiles) {
      for (const {
          fileName
        } of updatedSourceFile.libReferenceDirectives) {
        libReferenceDirectiveFileNames.add(fileName);
      }
      for (const {
          fileName
        } of updatedSourceFile.typeReferenceDirectives) {
...
function preserveSymbols(node, otherNode, options) {
  if (node === otherNode)
    return node;
  node._symbol = getSymbolAtLocation(Object.assign(Object.assign({}, options), {
    node: otherNode
  }));
  return node;
}
...
function preserveParents(node, options) {
  return tsCloneNode.setParentNodes(node, {
...
function visitNode$2(_a) {
  ...
  if (options.typescript.isClassDeclaration(node)) {
    return visitClassDeclaration$1(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (options.typescript.isClassExpression(node)) {
    return visitClassExpression$1(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (options.typescript.isFunctionDeclaration(node)) {
    return visitFunctionDeclaration$1(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (options.typescript.isFunctionExpression(node)) {
    return visitFunctionExpression$1(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (options.typescript.isEnumDeclaration(node)) {
    return visitEnumDeclaration$1(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (options.typescript.isInterfaceDeclaration(node)) {
    return visitInterfaceDeclaration$1(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (options.typescript.isTypeAliasDeclaration(node)) {
    return visitTypeAliasDeclaration$1(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (options.typescript.isModuleDeclaration(node)) {
    return visitModuleDeclaration$2(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (options.typescript.isVariableStatement(node)) {
    return visitVariableStatement$1(Object.assign(Object.assign({}, options), {
      node
    }));
  } else {
...
function ensureNoDeclareModifierTransformer(options) {
  ...
  // Prepare some VisitorOptions
  const visitorOptions = Object.assign(Object.assign({}, options), {
    childContinuation: (node) => typescript.visitEachChild(node, nextNode => visitNode$2(Object.assign(Object.assign({}, visitorOptions), {
      node: nextNode
    })), context),
    continuation: (node) => visitNode$2(Object.assign(Object.assign({}, visitorOptions), {
      node
    }))
  });
...
function visitNode$3(_a) {
  ...
  if (options.typescript.isImportDeclaration(node)) {
    return visitImportDeclaration$1(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (options.typescript.isExportDeclaration(node)) {
    return visitExportDeclaration$1(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (options.typescript.isExportAssignment(node)) {
    return visitExportAssignment$1(Object.assign(Object.assign({}, options), {
      node
    }));
  } else {
    // Only consider root-level statements here
  ...
  for (const [module, names] of defaultImportsFromModulesMap) {
    ...
    for (const name of names) {
      importDeclarationsForModule.push(preserveParents(compatFactory.createImportDeclaration(undefined, undefined, isNodeFactory(compatFactory) ?
        ...
        compatFactory.createImportClause(compatFactory.createIdentifier(name), undefined, false), compatFactory.createStringLiteral(ensureHasLeadingDotAndPosix(module))), {
        typescript
      }));
    }
  }
  ...
  for (const [module, names] of namespaceImportsFromModulesMap) {
    ...
    for (const name of names) {
      importDeclarationsForModule.push(preserveParents(compatFactory.createImportDeclaration(undefined, undefined, isNodeFactory(compatFactory) ?
        ...
        compatFactory.createImportClause(undefined, compatFactory.createNamespaceImport(compatFactory.createIdentifier(name)), false), compatFactory.createStringLiteral(ensureHasLeadingDotAndPosix(module))), {
        typescript
      }));
    }
  }
    ...
    for (const collection of collections) {
      ...
      importDeclarationsForModule.push(preserveParents(compatFactory.createImportDeclaration(undefined, undefined, isNodeFactory(compatFactory) ?
        ...
        compatFactory.createImportClause(undefined, compatFactory.createNamedImports(collection.map(record => record.propertyName !== record.alias ?
          compatFactory.createImportSpecifier(compatFactory.createIdentifier(record.propertyName), compatFactory.createIdentifier(record.alias)) :
          compatFactory.createImportSpecifier(undefined, compatFactory.createIdentifier(record.alias)))), false), compatFactory.createStringLiteral(ensureHasLeadingDotAndPosix(module))), {
        typescript
      }));
    }
  }
...
function getMergedExportDeclarationsForModules(sourceFile, compatFactory, typescript) {
  ...
  for (const [specifier, exportedBindings] of moduleSpecifierToAliasedExportedBindings) {
    ...
    exportDeclarationsForModule.push(preserveParents(isNodeFactory(compatFactory) ?
      ...
      compatFactory.createExportDeclaration(undefined, undefined, compatFactory.createNamedExports(exportSpecifiers), specifier == null ? undefined : compatFactory.createStringLiteral(ensureHasLeadingDotAndPosix(specifier))), {
        typescript
      }));
  }
  ...
  for (const [specifier, names] of namedNamespaceExportsFromModulesMap) {
    ...
    for (const name of names) {
      exportDeclarationsForModule.push(preserveParents(isNodeFactory(compatFactory) ?
        ...
        compatFactory.createExportDeclaration(undefined, undefined, compatFactory.createNamespaceExport(compatFactory.createIdentifier(name)), compatFactory.createStringLiteral(ensureHasLeadingDotAndPosix(specifier))), {
          typescript
        }));
    }
  }
...
function statementMerger({
  markAsModuleIfNeeded
}) {
  return options => {
    const {
      compatFactory,
    ...
    const visitorOptions = Object.assign(Object.assign({}, options), {
      ...
      childContinuation: (node) => typescript.visitEachChild(node, nextNode => visitNode$3(Object.assign(Object.assign({}, visitorOptions), {
        node: nextNode
      })), context),
      continuation: (node) => visitNode$3(Object.assign(Object.assign({}, visitorOptions), {
        node
      }))
    });
    ...
    result = preserveMeta(isNodeFactory(compatFactory) ?
      compatFactory.updateSourceFile(result, [
        ...
        ...otherStatements,
        ...exportDeclarations,
        ...(importExportCount === 0 && markAsModuleIfNeeded ? // Create an 'export {}' declaration to mark the declaration file as module-based if it has no imports or exports
...
function generateExportDeclarations(options, exportDeclarations = []) {
  ...
  for (const symbol of exportedSymbols) {
    ...
    const generatedModuleSpecifier = symbol.moduleSpecifier == null ?
      undefined :
      generateModuleSpecifier(Object.assign(Object.assign({}, options), {
    ...
    if ("isNamespaceExport" in symbol) {
      ...
      // Otherwise, recursively add all exports for the reexported module
        ...
        generateExportDeclarations(Object.assign(Object.assign({}, options), {
    ...
    // Otherwise, we can just add an ExportDeclaration with an ExportSpecifier
      ...
      exportDeclarations.push(preserveParents(isNodeFactory(compatFactory) ?
        ...
        compatFactory.createExportDeclaration(undefined, undefined, compatFactory.createNamedExports([exportSpecifier]), symbol.moduleSpecifier == null || generatedModuleSpecifier == null || matchingSourceFile != null ?
          undefined :
          compatFactory.createStringLiteral(generatedModuleSpecifier), false), {
          typescript
        }));
...
function visitExportDeclaration$2(options) {
  ...
  if (contResult.exportClause == null) {
    ...
    return generateExportDeclarations(Object.assign(Object.assign({}, options), {
      typescript,
      sourceFile: matchingSourceFile
    }));
  }
  ...
  // Instead, we must declare inline the namespace and add an ExportDeclaration with a named export for it
    ...
    options.prependNodes(preserveParents(compatFactory.createModuleDeclaration(undefined, ensureHasDeclareModifier(undefined, compatFactory, typescript), compatFactory.createIdentifier(contResult.exportClause.name.text), compatFactory.createModuleBlock([
      ...options.includeSourceFile(matchingSourceFile, {
        allowDuplicate: true,
        lexicalEnvironment: cloneLexicalEnvironment(),
        transformers: [ensureNoDeclareModifierTransformer, statementMerger({
          markAsModuleIfNeeded: false
        })]
      })
    ]), typescript.NodeFlags.Namespace), options), preserveParents(isNodeFactory(compatFactory) ?
...
function visitImportTypeNode$1(options) {
  ...
  if (contResult.qualifier == null) {
    ...
    options.prependNodes(preserveParents(compatFactory.createModuleDeclaration(undefined, ensureHasDeclareModifier(undefined, compatFactory, typescript), compatFactory.createIdentifier(namespaceName), compatFactory.createModuleBlock([
      ...options.includeSourceFile(matchingSourceFile, {
        allowDuplicate: true,
        lexicalEnvironment: cloneLexicalEnvironment(),
        transformers: [ensureNoDeclareModifierTransformer, statementMerger({
    ...
    ]), typescript.NodeFlags.Namespace), options));
...
function getAliasedDeclaration(options) {
  ...
  let symbol;
  try {
    symbol = node == null ? undefined : isSymbol(node) ? node : getSymbolAtLocation(Object.assign(Object.assign({}, options), {
      node
    }));
  } catch (_a) {
...
function getBestDeclaration(options) {
  ...
  if (options.typescript.isStringLiteralLike(moduleSpecifier)) {
    if (isSameChunk(Object.assign(Object.assign({}, options), {
...
function generateUniqueBinding(lexicalEnvironment, candidate) {
  ...
  if (lexicalEnvironment.bindings.has(candidate)) {
    const {
      value
    } = lexicalEnvironment.bindings.get(candidate);
  ...
  switch (declaration === null || declaration === void 0 ? void 0 : declaration.kind) {
    ...
    case typescript.SyntaxKind.ClassExpression: {
      return [
        ...
        preserveParents(compatFactory.createImportEqualsDeclaration(undefined, undefined, compatFactory.createIdentifier(name), compatFactory.createQualifiedName(compatFactory.createIdentifier(moduleBinding), compatFactory.createIdentifier(propertyName))), {
          typescript
        })
      ];
    }
    ...
    default: {
      return [
        preserveParents(compatFactory.createTypeAliasDeclaration(undefined, undefined, compatFactory.createIdentifier(name), undefined, compatFactory.createTypeReferenceNode(compatFactory.createIdentifier(propertyName), undefined)), {
          typescript
        })
      ];
    }
    ...
    if (matchedNamedExport != null) {
      ...
      for (const namespaceExport of exportedSymbolsArr.filter((exportedSymbol) => "isNamespaceExport" in exportedSymbol)) {
        ...
        if (sourceFile != null && !seenSourceFiles.has(sourceFile.fileName)) {
          const recursiveResult = locateExportedSymbolForSourceFile(options, Object.assign(Object.assign({}, context), {
...
function visitImportSpecifier(options) {
  ...
  const exportedSymbol = propertyName.text === "default" ?
    locateExportedSymbolForSourceFile({
      ...
      sourceFile: payload.matchingSourceFile.fileName
    })) :
    locateExportedSymbolForSourceFile({
      defaultExport: false,
  ...
  if (exportedSymbol != null) {
    ...
    if (exportedSymbol.moduleSpecifier != null &&
      ...
      const declaration = getAliasedDeclaration(Object.assign(Object.assign({}, options), {
        node: contResult.propertyName
      }));
...
function visitExportSpecifier(options) {
  ...
  if (exportedSymbol != null) {
    ...
    if (exportedSymbol.moduleSpecifier != null &&
      ...
      return preserveMeta(compatFactory.updateExportSpecifier(contResult, exportedSymbol.propertyName.text === contResult.name.text ? undefined : compatFactory.createIdentifier(exportedSymbol.propertyName.text), compatFactory.createIdentifier(contResult.name.text)), contResult, options);
    }
  }
  ...
  if (defaultExportedSymbol != null) {
    ...
    if (defaultExportedSymbol.moduleSpecifier != null &&
      ...
      options.prependNodes(preserveParents(compatFactory.createImportDeclaration(undefined, undefined, isNodeFactory(compatFactory) ?
        ...
        compatFactory.createImportClause(compatFactory.createIdentifier(contResult.name.text), undefined, false), compatFactory.createStringLiteral(generatedModuleSpecifier)), {
        typescript
      }));
    }
...
function visitNamespaceImport(options) {
  ...
  // Otherwise, prepend the nodes for the SourceFile in a namespace declaration
  options.prependNodes(preserveParents(compatFactory.createModuleDeclaration(undefined, ensureHasDeclareModifier(undefined, compatFactory, typescript), compatFactory.createIdentifier(contResult.name.text), compatFactory.createModuleBlock([
...
function visitSourceFile(options) {
  for (const otherEntrySourceFileForChunk of options.otherEntrySourceFilesForChunk) {
    options.prependNodes(...options.includeSourceFile(otherEntrySourceFileForChunk, {
      allowExports: true
    }));
  }
...
function visitNode$4(_a) {
  ...
  if (options.typescript.isSourceFile(node)) {
    return visitSourceFile(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (options.typescript.isExportDeclaration(node)) {
    return visitExportDeclaration$2(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (options.typescript.isImportDeclaration(node)) {
    return visitImportDeclaration$2(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (options.typescript.isImportTypeNode(node)) {
    return visitImportTypeNode$1(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (options.typescript.isImportClause(node)) {
    return visitImportClause(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (options.typescript.isNamespaceImport(node)) {
    return visitNamespaceImport(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (options.typescript.isImportSpecifier(node)) {
    return visitImportSpecifier(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (options.typescript.isExportSpecifier(node)) {
    return visitExportSpecifier(Object.assign(Object.assign({}, options), {
      node
    }));
  } else {
...
function getNodePlacementQueue({
  typescript
}) {
...
function visitNode$5(_a) {
  ...
  if (options.typescript.isClassDeclaration(node)) {
    return visitClassDeclaration$2(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (options.typescript.isClassExpression(node)) {
    return visitClassExpression$2(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (options.typescript.isFunctionDeclaration(node)) {
    return visitFunctionDeclaration$2(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (options.typescript.isFunctionExpression(node)) {
    return visitFunctionExpression$2(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (options.typescript.isEnumDeclaration(node)) {
    return visitEnumDeclaration$2(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (options.typescript.isInterfaceDeclaration(node)) {
    return visitInterfaceDeclaration$2(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (options.typescript.isTypeAliasDeclaration(node)) {
    return visitTypeAliasDeclaration$2(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (options.typescript.isModuleDeclaration(node)) {
    return visitModuleDeclaration$3(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (options.typescript.isVariableStatement(node)) {
    return visitVariableStatement$2(Object.assign(Object.assign({}, options), {
      node
    }));
  } else {
    // Only consider root-level statements here
...
function ensureNoExportModifierTransformer(options) {
  ...
  // Prepare some VisitorOptions
  const visitorOptions = Object.assign(Object.assign({}, options), {
    childContinuation: (node) => typescript.visitEachChild(node, nextNode => visitNode$5(Object.assign(Object.assign({}, visitorOptions), {
      node: nextNode
    })), context),
    continuation: (node) => visitNode$5(Object.assign(Object.assign({}, visitorOptions), {
      node
    }))
  });
  const result = preserveMeta(typescript.visitEachChild(sourceFile, nextNode => visitorOptions.continuation(nextNode), context), sourceFile, options);
...
function visitNode$6(_a) {
  ...
  if (options.typescript.isExportDeclaration(node)) {
    return visitExportDeclaration$3(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (options.typescript.isExportAssignment(node)) {
    return visitExportAssignment$2(Object.assign(Object.assign({}, options), {
      node
    }));
  } else {
    // Preserve the node
...
function noExportDeclarationTransformer(options) {
  ...
  const nodePlacementQueue = getNodePlacementQueue({
    typescript
  });
  // Prepare some VisitorOptions
  const visitorOptions = Object.assign(Object.assign(Object.assign({}, options), nodePlacementQueue), {
    childContinuation: (node) => typescript.visitEachChild(node, nextNode => nodePlacementQueue.wrapVisitResult(visitNode$6(Object.assign(Object.assign({}, visitorOptions), {
      node: nextNode
    }))), context),
    continuation: (node) => nodePlacementQueue.wrapVisitResult(visitNode$6(Object.assign(Object.assign({}, visitorOptions), {
      node
    })))
  });
  const result = preserveMeta(typescript.visitEachChild(sourceFile, nextNode => visitorOptions.continuation(nextNode), context), sourceFile, options);
...
function moduleMerger(...transformers) {
  return options => {
    ...
    const visitorOptions = Object.assign(Object.assign(Object.assign({}, options), nodePlacementQueue), {
      transformers,
      payload: undefined,
      childContinuation: (node, payload) => typescript.visitEachChild(node, nextNode => nodePlacementQueue.wrapVisitResult(visitNode$4(Object.assign(Object.assign({}, visitorOptions), {
        payload,
        node: nextNode
      }))), context),
      continuation: (node, payload) => nodePlacementQueue.wrapVisitResult(visitNode$4(Object.assign(Object.assign({}, visitorOptions), {
        payload,
        node
      }))),
      ...
      includeSourceFile(sourceFileToInclude, _a = {}) {
        var {
          allowDuplicate = false, allowExports = options.otherEntrySourceFilesForChunk.some(otherEntrySourceFileForChunk => otherEntrySourceFileForChunk.fileName === sourceFileToInclude.fileName), transformers: extraTransformers = []
        } = _a, otherOptions = tslib.__rest(_a, ["allowDuplicate", "allowExports", "transformers"]);
        ...
        const allTransformers = allowExports ? [...transformers, ...extraTransformers] : [
          ...transformers,
...
function addBindingToLexicalEnvironment(lexicalEnvironment, originalSourceFileName, value, oldValue = value) {
  lexicalEnvironment.bindings.set(oldValue, {
...
function deconflictBindingElement(options) {
  ...
  if (typescript.isIdentifier(node.name)) {
    ...
    // Otherwise, deconflict it
    nameContResult = continuation(node.name, {
      lexicalEnvironment
    });
  }
  const propertyNameContResult = node.propertyName == null ? undefined : typescript.isIdentifier(node.propertyName) ? node.propertyName : continuation(node.propertyName, {
    lexicalEnvironment
  });
  const initializerContResult = node.initializer == null ? undefined : continuation(node.initializer, {
    lexicalEnvironment
  });
...
function getIdForStructurallyEqualNode(options) {
  if (options.typescript.isImportSpecifier(options.node)) {
    return getIdForImportSpecifier(Object.assign(Object.assign({}, options), {
      node: options.node.name
    }));
  } else if (options.typescript.isNamespaceImport(options.node)) {
    return getIdForNamespaceImportName(Object.assign(Object.assign({}, options), {
      node: options.node.name
    }));
  } else if (options.typescript.isImportClause(options.node) && options.node.name != null) {
    return getIdForImportedName(Object.assign(Object.assign({}, options), {
      node: options.node.name
    }));
  } else {
    return undefined;
...
function getIdForNode(options) {
  ...
  if (options.typescript.isExportSpecifier(options.node)) {
    ...
    if (aliasedDeclaration != null && aliasedDeclaration !== options.node) {
      return getIdForNode(Object.assign(Object.assign({}, options), {
        node: aliasedDeclaration
      }));
    }
  }
  ...
  if (importRelatedId != null) {
    ...
    if (parent != null) {
      const parentImportRelatedId = getIdForStructurallyEqualNode(Object.assign(Object.assign({}, options), {
        node: parent
      }));
...
function deconflictClassDeclaration(options) {
  const {
    ...
    compatFactory,
    sourceFile,
    declarationToDeconflictedBindingMap
  } = options;
  let nameContResult;
  if (node.name != null) {
  ...
  const nextContinuationOptions = {
    lexicalEnvironment: cloneLexicalEnvironment(lexicalEnvironment)
  };
  ...
  const membersContResult = node.members.map(member => continuation(member, {
    lexicalEnvironment
  }));
...
function deconflictEnumMember(options) {
  ...
  const nameContResult = typescript.isIdentifier(node.name) ? node.name : continuation(node.name, {
    lexicalEnvironment
  });
  const initializerContResult = node.initializer == null ? undefined : continuation(node.initializer, {
    lexicalEnvironment
...
function deconflictExportSpecifier(options) {
  ...
  const propertyNameContResult = continuation(propertyName, {
    lexicalEnvironment
  });
...
function deconflictIdentifier(options) {
  ...
  const id = getIdForNode(Object.assign(Object.assign({}, options), {
    node: (_a = getBestDeclaration(options)) !== null && _a !== void 0 ? _a : node
  }));
...
function deconflictImportClause(options) {
  ...
  const namedBindingsContResult = node.namedBindings == null ? undefined : continuation(node.namedBindings, {
    lexicalEnvironment
  });
...
function deconflictParameterDeclaration(options) {
  ...
  const typeContResult = node.type == null ? undefined : continuation(node.type, {
    lexicalEnvironment
  });
  const initializerContResult = node.initializer == null ? undefined : continuation(node.initializer, {
    lexicalEnvironment
...
function deconflictTypeParameterDeclaration(options) {
  ...
  const constraintContResult = node.constraint == null ? undefined : continuation(node.constraint, {
    lexicalEnvironment
  });
  const defaultContResult = node.default == null ? undefined : continuation(node.default, {
    lexicalEnvironment
  });
...
function deconflictImportTypeNode(options) {
  ...
  const argumentContResult = continuation(node.argument, {
    lexicalEnvironment
  });
  const typeArgumentsContResult = node.typeArguments == null ? undefined : node.typeArguments.map(typeArgument => continuation(typeArgument, {
    lexicalEnvironment
  }));
...
function deconflictNode(_a) {
  ...
  if (options.typescript.isBindingElement(node)) {
    return deconflictBindingElement(Object.assign({
      node
    }, options));
  } else if (options.typescript.isClassDeclaration(node)) {
    return deconflictClassDeclaration(Object.assign({
      node
    }, options));
  } else if (options.typescript.isClassExpression(node)) {
    return deconflictClassExpression(Object.assign({
      node
    }, options));
  } else if (options.typescript.isEnumDeclaration(node)) {
    return deconflictEnumDeclaration(Object.assign({
      node
    }, options));
  } else if (options.typescript.isEnumMember(node)) {
    return deconflictEnumMember(Object.assign({
      node
    }, options));
  } else if (options.typescript.isExportSpecifier(node)) {
    return deconflictExportSpecifier(Object.assign({
      node
    }, options));
  } else if (options.typescript.isFunctionDeclaration(node)) {
    return deconflictFunctionDeclaration(Object.assign({
      node
    }, options));
  } else if (options.typescript.isFunctionExpression(node)) {
    return deconflictFunctionExpression(Object.assign({
      node
    }, options));
  } else if (options.typescript.isConstructorDeclaration(node)) {
    return deconflictConstructorDeclaration(Object.assign({
      node
    }, options));
  } else if (options.typescript.isFunctionTypeNode(node)) {
    return deconflictFunctionTypeNode(Object.assign({
      node
    }, options));
  } else if (options.typescript.isGetAccessorDeclaration(node)) {
    return deconflictGetAccessorDeclaration(Object.assign({
      node
    }, options));
  } else if (options.typescript.isIdentifier(node)) {
    return deconflictIdentifier(Object.assign({
      node
    }, options));
  } else if (options.typescript.isImportClause(node)) {
    return deconflictImportClause(Object.assign({
      node
    }, options));
  } else if (options.typescript.isImportSpecifier(node)) {
    return deconflictImportSpecifier(Object.assign({
      node
    }, options));
  } else if (options.typescript.isInterfaceDeclaration(node)) {
    return deconflictInterfaceDeclaration(Object.assign({
      node
    }, options));
  }
  // MappedTypeNodes may not be part of the current Typescript version, hence the optional call
    ...
    return deconflictMappedTypeNode(Object.assign({
      node
    }, options));
  } else if (options.typescript.isMethodDeclaration(node)) {
    return deconflictMethodDeclaration(Object.assign({
      node
    }, options));
  } else if (options.typescript.isIndexSignatureDeclaration(node)) {
    return deconflictIndexSignatureDeclaration(Object.assign({
      node
    }, options));
  } else if (options.typescript.isMethodSignature(node)) {
    return deconflictMethodSignature(Object.assign({
      node
    }, options));
  } else if (options.typescript.isModuleDeclaration(node)) {
    return deconflictModuleDeclaration(Object.assign({
      node
    }, options));
  } else if (options.typescript.isNamespaceImport(node)) {
    return deconflictNamespaceImport(Object.assign({
      node
    }, options));
  } else if (options.typescript.isParameter(node)) {
    return deconflictParameterDeclaration(Object.assign({
      node
    }, options));
  } else if (options.typescript.isPropertyAssignment(node)) {
    return deconflictPropertyAssignment(Object.assign({
      node
    }, options));
  } else if (options.typescript.isPropertyDeclaration(node)) {
    return deconflictPropertyDeclaration(Object.assign({
      node
    }, options));
  } else if (options.typescript.isPropertySignature(node)) {
    return deconflictPropertySignature(Object.assign({
      node
    }, options));
  } else if (options.typescript.isSetAccessorDeclaration(node)) {
    return deconflictSetAccessorDeclaration(Object.assign({
      node
    }, options));
  } else if (options.typescript.isTypeAliasDeclaration(node)) {
    return deconflictTypeAliasDeclaration(Object.assign({
      node
    }, options));
  } else if (options.typescript.isImportTypeNode(node)) {
    return deconflictImportTypeNode(Object.assign({
      node
    }, options));
  } else if (options.typescript.isTypeParameterDeclaration(node)) {
    return deconflictTypeParameterDeclaration(Object.assign({
      node
    }, options));
  } else if (options.typescript.isVariableDeclaration(node)) {
    return deconflictVariableDeclaration(Object.assign({
      node
    }, options));
  } else
    return options.childContinuation(node, {
...
function deconflicter(options) {
  ...
  // Prepare some VisitorOptions
  const visitorOptions = Object.assign(Object.assign({}, options), {
    childContinuation: (node, continuationOptions) => typescript.visitEachChild(node, nextNode => deconflictNode(Object.assign(Object.assign(Object.assign({}, visitorOptions), continuationOptions), {
      node: nextNode
    })), context),
    continuation: (node, continuationOptions) => deconflictNode(Object.assign(Object.assign(Object.assign({}, visitorOptions), continuationOptions), {
      node
    }))
  });
  const result = preserveMeta(typescript.visitEachChild(sourceFile, nextNode => visitorOptions.continuation(nextNode, {
    lexicalEnvironment
  }), context), sourceFile, options);
  transformationLog === null || transformationLog === void 0 ? void 0 : transformationLog.finish(result);
  fullBenchmark === null || fullBenchmark === void 0 ? void 0 : fullBenchmark.finish();
  return result;
}
...
function visitNode$7(_a) {
  ...
  if (options.typescript.isClassDeclaration(node)) {
    return visitClassDeclaration$3(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (options.typescript.isClassExpression(node)) {
    return visitClassExpression$3(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (options.typescript.isFunctionDeclaration(node)) {
    return visitFunctionDeclaration$3(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (options.typescript.isFunctionExpression(node)) {
    return visitFunctionExpression$3(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (options.typescript.isEnumDeclaration(node)) {
    return visitEnumDeclaration$3(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (options.typescript.isInterfaceDeclaration(node)) {
    return visitInterfaceDeclaration$3(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (options.typescript.isTypeAliasDeclaration(node)) {
    return visitTypeAliasDeclaration$3(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (options.typescript.isModuleDeclaration(node)) {
    return visitModuleDeclaration$4(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (options.typescript.isVariableStatement(node)) {
    return visitVariableStatement$3(Object.assign(Object.assign({}, options), {
      node
    }));
  } else {
    // Only consider root-level statements here
...
function ensureDeclareModifierTransformer(options) {
  ...
  // Prepare some VisitorOptions
  const visitorOptions = Object.assign(Object.assign({}, options), {
    childContinuation: (node) => typescript.visitEachChild(node, nextNode => visitNode$7(Object.assign(Object.assign({}, visitorOptions), {
      node: nextNode
    })), context),
    continuation: (node) => visitNode$7(Object.assign(Object.assign({}, visitorOptions), {
      node
    }))
  });
  const result = preserveMeta(typescript.visitEachChild(sourceFile, nextNode => visitorOptions.continuation(nextNode), context), sourceFile, options);
...
function visitModuleDeclaration$5({
  node,
  typescript
}) {
  if (node.body == null)
...
function visitNode$8(_a) {
  ...
  if (options.typescript.isModuleDeclaration(node)) {
    return visitModuleDeclaration$5(Object.assign(Object.assign({}, options), {
      node
    }));
  } else {
    return options.childContinuation(node);
...
function moduleBlockExtractor(options) {
  ...
  // Prepare some VisitorOptions
  const visitorOptions = Object.assign(Object.assign({}, options), {
    childContinuation: (node) => typescript.visitEachChild(node, nextNode => visitNode$8(Object.assign(Object.assign({}, visitorOptions), {
      node: nextNode
    })), context),
    continuation: (node) => visitNode$8(Object.assign(Object.assign({}, visitorOptions), {
      node
    }))
  });
  const result = preserveMeta(typescript.visitEachChild(sourceFile, nextNode => visitorOptions.continuation(nextNode), context), sourceFile, options);
...
function checkArrayBindingPattern({
  node,
  continuation
}) {
  const referencedIdentifiers = [];
...
function checkObjectBindingPattern({
  node,
  continuation
}) {
  const referencedIdentifiers = [];
...
function checkGetAccessorDeclaration({
  node,
  continuation,
  typescript
}) {
...
function checkSetAccessorDeclaration({
  node,
  continuation,
  typescript
}) {
...
function checkParameterDeclaration({
  node,
  continuation,
  typescript
}) {
...
function checkVariableDeclarationList({
  node,
  continuation
}) {
  const referencedIdentifiers = [];
...
function checkIndexedAccessTypeNode({
  node,
  continuation
}) {
  const referencedIdentifiers = [];
...
function checkTemplateLiteralTypeNode({
  node,
  continuation
}) {
  const referencedIdentifiers = [];
...
function checkTemplateLiteralTypeSpan({
  node,
  continuation
}) {
  const referencedIdentifiers = [];
...
function checkNode(_a) {
  ...
  var {
    node,
    originalNode
  } = _a, options = tslib.__rest(_a, ["node", "originalNode"]);
  if (options.typescript.isArrayBindingPattern(node)) {
    return checkArrayBindingPattern(Object.assign({
      node,
      originalNode
    }, options));
  } else if (options.typescript.isObjectBindingPattern(node)) {
    return checkObjectBindingPattern(Object.assign({
      node,
      originalNode
    }, options));
  } else if (options.typescript.isParameter(node)) {
    return checkParameterDeclaration(Object.assign({
      node,
      originalNode
    }, options));
  } else if (options.typescript.isQualifiedName(node)) {
    return checkQualifiedName(Object.assign({
      node,
      originalNode
    }, options));
  } else if (options.typescript.isBindingElement(node)) {
    return checkBindingElement(Object.assign({
      node,
      originalNode
    }, options));
  } else if (options.typescript.isMethodDeclaration(node)) {
    return checkMethodDeclaration(Object.assign({
      node,
      originalNode
    }, options));
  } else if (options.typescript.isMethodSignature(node)) {
    return checkMethodSignature(Object.assign({
      node,
      originalNode
    }, options));
  } else if (options.typescript.isGetAccessorDeclaration(node)) {
    return checkGetAccessorDeclaration(Object.assign({
      node,
      originalNode
    }, options));
  } else if (options.typescript.isSetAccessorDeclaration(node)) {
    return checkSetAccessorDeclaration(Object.assign({
      node,
      originalNode
    }, options));
  } else if (options.typescript.isPropertyAccessExpression(node)) {
    return checkPropertyAccessExpression(Object.assign({
      node,
      originalNode
    }, options));
  } else if (options.typescript.isPropertyDeclaration(node)) {
    return checkPropertyDeclaration(Object.assign({
      node,
      originalNode
    }, options));
  } else if (options.typescript.isPropertySignature(node)) {
    return checkPropertySignature(Object.assign({
      node,
      originalNode
    }, options));
  } else if (options.typescript.isClassDeclaration(node)) {
    return checkClassDeclaration(Object.assign({
      node,
      originalNode
    }, options));
  } else if (options.typescript.isClassExpression(node)) {
    return checkClassExpression(Object.assign({
      node,
      originalNode
    }, options));
  } else if (options.typescript.isFunctionDeclaration(node)) {
    return checkFunctionDeclaration(Object.assign({
      node,
      originalNode
    }, options));
  } else if (options.typescript.isFunctionExpression(node)) {
    return checkFunctionExpression(Object.assign({
      node,
      originalNode
    }, options));
  } else if (options.typescript.isInterfaceDeclaration(node)) {
    return checkInterfaceDeclaration(Object.assign({
      node,
      originalNode
    }, options));
  } else if (options.typescript.isEnumDeclaration(node)) {
    return checkEnumDeclaration(Object.assign({
      node,
      originalNode
    }, options));
  } else if (options.typescript.isTypeAliasDeclaration(node)) {
    return checkTypeAliasDeclaration(Object.assign({
      node,
      originalNode
    }, options));
  } else if (options.typescript.isIndexedAccessTypeNode(node)) {
    return checkIndexedAccessTypeNode(Object.assign({
      node,
      originalNode
    }, options));
  } else if (options.typescript.isVariableStatement(node)) {
    return checkVariableStatement(Object.assign({
      node,
      originalNode
    }, options));
  } else if (options.typescript.isVariableDeclarationList(node)) {
    return checkVariableDeclarationList(Object.assign({
      node,
      originalNode
    }, options));
  } else if (options.typescript.isVariableDeclaration(node)) {
    return checkVariableDeclaration(Object.assign({
      node,
      originalNode
    }, options));
  } else if (options.typescript.isExportDeclaration(node)) {
    return checkExportDeclaration(Object.assign({
      node,
      originalNode
    }, options));
  } else if (options.typescript.isExportAssignment(node)) {
    return checkExportAssignment(Object.assign({
      node,
      originalNode
    }, options));
  } else if (options.typescript.isExportSpecifier(node)) {
    return checkExportSpecifier(Object.assign({
      node,
      originalNode
    }, options));
  } else if (options.typescript.isModuleDeclaration(node)) {
    return checkModuleDeclaration(Object.assign({
      node,
      originalNode
    }, options));
  } else if (options.typescript.isIdentifier(node)) {
    return checkIdentifier(Object.assign({
      node,
      originalNode
    }, options));
  } else if (isTemplateLiteralTypeNode(node, options.typescript)) {
    return checkTemplateLiteralTypeNode(Object.assign({
      node,
      originalNode
    }, options));
  } else if ((_c = (_b = options.typescript).isTemplateLiteralTypeSpan) === null || _c === void 0 ? void 0 : _c.call(_b, node)) {
    return checkTemplateLiteralTypeSpan(Object.assign({
      node,
      originalNode
    }, options));
  } else if (options.typescript.isTypeReferenceNode(node)) {
    return checkTypeReferenceNode(Object.assign({
      node,
      originalNode
    }, options));
  } else {
...
function isReferenced(_a) {
  var {
    seenNodes = new Set()
  } = _a, options = tslib.__rest(_a, ["seenNodes"]);
  ...
  // Compute the result
  const result = referencingNodes.length > 0 && referencingNodes.some(referencingNode => isReferenced(Object.assign(Object.assign({}, options), {
    seenNodes,
    ...
    const visitorOptions = Object.assign(Object.assign({}, options), {
      ...
      childContinuation: (node) => {
        ...
        options.typescript.forEachChild(node, nextNode => {
          referencedIdentifiers.push(...checkNode(Object.assign(Object.assign({}, visitorOptions), {
            node: nextNode
          })));
        });
        return referencedIdentifiers;
      },
      continuation: (node) => checkNode(Object.assign(Object.assign({}, visitorOptions), {
        node
      }))
    });
    options.typescript.forEachChild(options.sourceFile, node => {
      checkNode(Object.assign(Object.assign({}, visitorOptions), {
        node
      }));
    });
  }
...
function visitClassDeclaration$4({
  node,
  continuation,
  compatFactory
}) {
...
function visitClassExpression$4({
  node,
  continuation,
  compatFactory
}) {
...
function visitFunctionDeclaration$4({
  node,
  continuation,
  compatFactory
}) {
...
function visitFunctionExpression$4({
  node,
  continuation,
  compatFactory
}) {
...
function visitEnumDeclaration$4({
  node,
  continuation,
  compatFactory
}) {
...
function visitTypeAliasDeclaration$4({
  node,
  continuation,
  compatFactory
}) {
...
function visitModuleDeclaration$6({
  node
}) {
  return node;
}

function visitExportDeclaration$4({
  node
}) {
  return node;
}

function visitExportAssignment$3({
  node
}) {
  return node;
}

function visitVariableStatement$4({
  node,
  continuation,
  compatFactory
}) {
...
function visitImportDeclaration$3({
  node,
  continuation,
  compatFactory
}) {
...
function visitImportSpecifier$1({
  node,
  continuation,
  compatFactory
}) {
...
function visitNamespaceImport$1({
  node,
  continuation,
  compatFactory
}) {
...
function visitArrayBindingPattern({
  node,
  continuation,
  compatFactory
}) {
...
function visitObjectBindingPattern({
  node,
  continuation,
  compatFactory
}) {
...
function visitNode$9(options) {
  ...
  if (typescript.isClassDeclaration(node)) {
    return visitClassDeclaration$4(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (typescript.isClassExpression(node)) {
    return visitClassExpression$4(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (typescript.isFunctionDeclaration(node)) {
    return visitFunctionDeclaration$4(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (typescript.isFunctionExpression(node)) {
    return visitFunctionExpression$4(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (typescript.isEnumDeclaration(node)) {
    return visitEnumDeclaration$4(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (typescript.isInterfaceDeclaration(node)) {
    return visitInterfaceDeclaration$4(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (typescript.isTypeAliasDeclaration(node)) {
    return visitTypeAliasDeclaration$4(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (typescript.isModuleDeclaration(node)) {
    return visitModuleDeclaration$6(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (typescript.isExportDeclaration(node)) {
    return visitExportDeclaration$4(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (typescript.isExportAssignment(node)) {
    return visitExportAssignment$3(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (typescript.isVariableStatement(node)) {
    return visitVariableStatement$4(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (typescript.isVariableDeclarationList(node)) {
    return visitVariableDeclarationList(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (typescript.isVariableDeclaration(node)) {
    return visitVariableDeclaration(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (typescript.isImportDeclaration(node)) {
    return visitImportDeclaration$3(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (typescript.isImportSpecifier(node)) {
    return visitImportSpecifier$1(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (typescript.isImportClause(node)) {
    return visitImportClause$1(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (typescript.isNamedImports(node)) {
    return visitNamedImports(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (typescript.isNamespaceImport(node)) {
    return visitNamespaceImport$1(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (typescript.isImportEqualsDeclaration(node)) {
    return visitImportEqualsDeclaration(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (typescript.isArrayBindingPattern(node)) {
    return visitArrayBindingPattern(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (typescript.isObjectBindingPattern(node)) {
    return visitObjectBindingPattern(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (typescript.isBindingElement(node)) {
    return visitBindingElement(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (typescript.isIdentifier(node)) {
    return visitIdentifier(Object.assign(Object.assign({}, options), {
      node
    }));
  } else {
...
function treeShaker(options) {
  ...
  const visitorOptions = Object.assign(Object.assign({}, options), {
    ...
    isReferenced: (node) => isReferenced(Object.assign(Object.assign({}, visitorOptions), {
      node
    })),
    continuation: (node) => visitNode$9(Object.assign(Object.assign({}, visitorOptions), {
      node
    }))
  });
...
function visitVariableStatement$5(options) {
  ...
  for (const declaration of node.declarationList.declarations) {
    const identifiers = traceIdentifiers(Object.assign(Object.assign({}, options), {
      node: declaration
    }));
    for (const identifier of identifiers) {
      const {
...
function visitNode$a(_a) {
  ...
  if (options.typescript.isClassDeclaration(node)) {
    return visitClassDeclaration$5(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (options.typescript.isClassExpression(node)) {
    return visitClassExpression$5(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (options.typescript.isFunctionDeclaration(node)) {
    return visitFunctionDeclaration$5(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (options.typescript.isFunctionExpression(node)) {
    return visitFunctionExpression$5(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (options.typescript.isEnumDeclaration(node)) {
    return visitEnumDeclaration$5(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (options.typescript.isInterfaceDeclaration(node)) {
    return visitInterfaceDeclaration$5(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (options.typescript.isTypeAliasDeclaration(node)) {
    return visitTypeAliasDeclaration$5(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (options.typescript.isModuleDeclaration(node)) {
    return visitModuleDeclaration$7(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (options.typescript.isVariableStatement(node)) {
    return visitVariableStatement$5(Object.assign(Object.assign({}, options), {
      node
    }));
  } else {
    // Only consider root-level statements here
...
function toExportDeclarationTransformer(options) {
  ...
  // Prepare some VisitorOptions
  const visitorOptions = Object.assign(Object.assign(Object.assign({}, options), nodePlacementQueue), {
    childContinuation: (node) => typescript.visitEachChild(node, nextNode => nodePlacementQueue.wrapVisitResult(visitNode$a(Object.assign(Object.assign({}, visitorOptions), {
      node: nextNode
    }))), context),
    continuation: (node) => nodePlacementQueue.wrapVisitResult(visitNode$a(Object.assign(Object.assign({}, visitorOptions), {
      node
    })))
  });
  let result = typescript.visitEachChild(sourceFile, nextNode => visitorOptions.continuation(nextNode), context);
...
function typeModuleReferenceIsAllowed({
  host,
  moduleSpecifier
}) {
...
function getTypeReferenceModuleFromFileName({
  ...
  for (const typeRoot of host.getTypeRoots()) {
    ...
    if (typeModuleReferenceIsAllowed({
        host,
        moduleSpecifier
      })) {
      return {
...
function getTypeReferenceModuleFromNode(options) {
  ...
  if (declarationSourceFile == null)
    return;
  const typeReference = getTypeReferenceModuleFromFileName(Object.assign(Object.assign({}, options), {
...
function visitIdentifier$1(options) {
  ...
  const typeReferenceModule = getTypeReferenceModuleFromNode(Object.assign(Object.assign({}, options), {
    node
  }));
...
function visitNode$b(_a) {
  ...
  if (options.typescript.isIdentifier(node)) {
    return visitIdentifier$1(Object.assign(Object.assign({}, options), {
      node
    }));
  } else {
    // Only consider root-level statements here
...
function typeReferenceCollector(options) {
  ...
  const visitorOptions = Object.assign(Object.assign({}, options), {
    ...
    childContinuation: (node) => typescript.forEachChild(node, nextNode => {
      visitNode$b(Object.assign(Object.assign({}, visitorOptions), {
        node: nextNode
      }));
    }),
    continuation: (node) => {
...
function mergeChunksWithAmbientDependencies({
  format = "esm",
  chunkFileNames,
  chunks,
  externalOption,
...
function emitDeclarations(options) {
  ...
  const normalizedChunks = chunks.map(chunk => normalizeChunk(chunk, Object.assign(Object.assign({}, options), {
    relativeOutDir
  })));
  ...
  const host = options.host.clone(Object.assign(Object.assign({}, options.host.getCompilationSettings()), {
  ...
  const sharedOptions = Object.assign(Object.assign({}, options), {
  ...
  for (const chunk of normalizedChunks) {
    ...
    if (chunk.entryModules.size < 1)
      continue;
    const bundleResult = bundleDeclarationsForChunk(Object.assign(Object.assign({}, sharedOptions), {
      chunk,
      declarationPaths,
      declarationMapPaths,
...
class ModuleResolutionHost {
  ...
  add(fileInput) {
    const file = Object.assign(Object.assign({}, fileInput), {
...
function resolveId(_a) {
  var {
    resolveCache
  } = _a, options = tslib.__rest(_a, ["resolveCache"]);
...
function ensureModuleTransformer({
  typescript,
  compatFactory,
  sourceFile
}) {
...
function visitImportDeclaration$4({
  node,
  typescript,
  host,
  sourceFile,
  ...
  if (resolvedModule != null) {
    addDependency(Object.assign(Object.assign({}, resolvedModule), {
      moduleSpecifier: node.moduleSpecifier.text
    }));
  }
}

function visitImportTypeNode$2({
  node,
  typescript,
  host,
  sourceFile,
...
function visitExportDeclaration$5({
  node,
  typescript,
  host,
  sourceFile,
...
function visitNode$c(_a) {
  ...
  if (options.typescript.isImportDeclaration(node)) {
    return visitImportDeclaration$4(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (options.typescript.isImportTypeNode(node)) {
    return visitImportTypeNode$2(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (options.typescript.isExportDeclaration(node)) {
    return visitExportDeclaration$5(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (options.typescript.isModuleDeclaration(node)) {
    return visitModuleDeclaration$8(Object.assign(Object.assign({}, options), {
      node
    }));
  } else if (options.shouldDeepTraverse) {
    return options.childContinuation(node);
...
function trackDependenciesTransformer(options) {
  ...
  const visitorOptions = Object.assign(Object.assign({}, options), {
    ...
    childContinuation: (node) => typescript.forEachChild(node, nextNode => {
      visitNode$c(Object.assign(Object.assign({}, visitorOptions), {
        node: nextNode
      }));
    }),
    continuation: (node) => {
...
class CompilerHost extends ModuleResolutionHost {
  ...
  getDependenciesForFileDeep(fileName, dependencies = new Set(), seenModules = new Set()) {
    ...
    if (localDependencies != null) {
      for (const dependency of localDependencies) {
        if (!dependenciesArr.some(({
            resolvedFileName,
            resolvedAmbientFileName
  ...
  add(fileInput, traceDependencies = true) {
    ...
    if (fileInput.fromRollup) {
      ...
      const transformedSourceFile = ensureModuleTransformer({
        typescript,
        compatFactory,
        sourceFile
      });
  ...
  clone(compilerOptions, fileNameFilter = () => true, overrides = {}) {
    return new CompilerHost(Object.assign(Object.assign(Object.assign({}, this.options), overrides), {
      parsedCommandLineResult: Object.assign(Object.assign({}, this.options.parsedCommandLineResult), {
        parsedCommandLine: Object.assign(Object.assign({}, this.getParsedCommandLine()), {
          fileNames: this.getParsedCommandLine().fileNames.filter(fileNameFilter),
          options: Object.assign(Object.assign({}, this.getCompilationSettings()), compilerOptions)
        })
      })
    }), this.printer, new Map([...this.sourceFiles.entries()].filter(([path]) => fileNameFilter(path))), new Map([...this.transformerDiagnostics.entries()].filter(([path]) => fileNameFilter(path))), new Map([...this.fileToVersionMap.entries()].filter(([path]) => fileNameFilter(path))), new Map([...this.sourceFileToDependenciesMap.entries()].filter(([path]) => fileNameFilter(path))), new Map([...this.files.entries()].filter(([path]) => fileNameFilter(path))));
  }
  ...
  getCustomTransformers(transformers = this.getTransformers()) {
    ...
    if (!this.allowTransformingDeclarations()) {
      return Object.assign(Object.assign({}, upgradedTransformers), {
  ...
  resolveModuleNames(moduleNames, containingFile) {
    ...
    for (const moduleName of moduleNames) {
      ...
      if (result != null && result.resolvedAmbientFileName != null) {
        resolvedModules.push(Object.assign(Object.assign({}, result), {
          resolvedFileName: result.resolvedAmbientFileName
        }));
  ...
  resolveTypeReferenceDirectives(typeReferenceDirectiveNames, containingFile) {
    ...
    for (const typeReferenceDirectiveName of typeReferenceDirectiveNames) {
      ...
      if (result != null && result.resolvedAmbientFileName != null) {
        resolvedTypeReferenceDirectives.push(Object.assign(Object.assign({}, result), {
          primary: true,
          resolvedFileName: result.resolvedAmbientFileName
        }));
      } else if (result != null && result.resolvedFileName != null) {
 ...
 */
function isBuildInfoOutputFile({
  name
}) {
  ...
  return {
    ...
    options(options) {
      ...
      if (pluginOptions.transpiler === "babel") {
        ...
        babelConfigFileFactory = getBabelConfig(Object.assign(Object.assign({}, sharedBabelConfigFactoryOptions), {
          phase: "file"
        }));
        babelConfigChunkFactory = getBabelConfig(Object.assign(Object.assign({}, sharedBabelConfigFactoryOptions), {
          phase: "chunk"
        }));
      }
    ...
    renderChunk(code, chunk, outputOptions) {
      ...
      return tslib.__awaiter(this, void 0, void 0, function* () {
        ...
        const {
          config
        } = babelConfigChunkFactory(chunk.fileName);
        ...
        const transpilationResult = yield core.transformAsync(updatedCode, Object.assign(Object.assign(Object.assign({}, config), {
    ...
    transform(code, fileInput) {
      ...
      return tslib.__awaiter(this, void 0, void 0, function* () {
        ...
        console.log("filter ? ", filter(normalizedFile));
        console.log("isBabelHelper ? ", isBabelHelper(normalizedFile));
        ...
        // console.log("hasJsonExtension ? ", hasJsonExtension);
        // console.log("isJsonLike ?", isJsonLike(code));
        ...
        if (hasJsonExtension) {
          ...
          console.log(`
            *******
            ...
            ${normalizedFile} as a json extension
            isJsInDisguise ?, ${isJsInDisguise}
            code:
            ${code}
        ...
        if (sourceDescription == null) {
          ...
          // Otherwise, pass it on to Babel to perform the rest of the transpilation steps
            ...
            const transpilationResult = yield core.transformAsync(sourceDescription.code, Object.assign(Object.assign({}, babelConfigResult.config), {
