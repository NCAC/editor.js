(function () {
  ...
  const _log = (labeled, msg, type = 'log', // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  async function sequence(chains, // eslint-disable-next-line @typescript-eslint/no-empty-function
  success = () => {}, // eslint-disable-next-line @typescript-eslint/no-empty-function
  fallback = () => {}) {
    /**
     * Decorator
     *
     ...
     * @returns {Promise}
     */
    async function waitNextBlock(chainData, successCallback, fallbackCallback) {
      try {
  ...
  function debounce(func, wait, immediate) {
    ...
    return () => {
      ...
      const context = this,
            // eslint-disable-next-line prefer-rest-params
      args = arguments; // eslint-disable-next-line @typescript-eslint/explicit-function-return-type
  ...
  class Dom {
    ...
    static get allInputsSelector() {
      ...
      return '[contenteditable], textarea, input:not([type]), ' + allowedInputTypes.map(type => `input[type="${type}"]`).join(', ');
    }
    /**
  ...
  class Paragraph {
    ...
    renderSettings() {
      const holder = document.createElement('DIV'); // do not add settings button, when only one level is configured
    ...
    onPaste(event) {
      ...
      let level = this.defaultLevel.number; // get data-level attribute
  ...
  class Tools extends Module {
    ...
    getToolSettings(toolName) {
      ...
      const config = settings[this.USER_SETTINGS.CONFIG] || {}; // Pass placeholder to default Block config
  ...
  const APIModules = [BlocksAPI, CaretAPI, EventsAPI, I18nAPI, API, InlineToolbarAPI, ListenersAPI, NotifierAPI, ReadOnlyAPI, SanitizerAPI, SaverAPI, SelectionAPI, StylesAPI, ToolbarAPI, TooltipAPI];
  ...
  class Flipper {
    ...
    isEventReadyForHandling(event) {
      const handlingKeyCodeList = [keyCodes.TAB, keyCodes.ENTER];
  ...
  class BlockSettings extends Module {
    ...
    get blockTunesButtons() {
      ...
      const toolSettings = this.nodes.toolSettings.querySelectorAll( // Select buttons and inputs
  ...
  class ConversionToolbar extends Module {
    ...
    make() {
      this.nodes.wrapper = Dom.make('div', [ConversionToolbar.CSS.conversionToolbarWrapper, ...(this.isRtl ? [this.Editor.UI.CSS.editorRtlFix] : [])]);
    ...
    async replaceWithBlock(replacingToolName) {
      /**
      ...
      const savedBlock = await this.Editor.BlockManager.currentBlock.save();
      const {
        INTERNAL_SETTINGS
      } = this.Editor.Tools;
  ...
  class MoveDownTune {
    ...
    handleClick(event, button) {
      const currentBlockIndex = this.api.blocks.getCurrentBlockIndex();
      const nextBlock = this.api.blocks.getBlockByIndex(currentBlockIndex + 1); // If Block is last do nothing
  ...
  class Block {
    ...
    get hasMedia() {
       ...
       * @type {string[]}
       */
      const mediaTags = ['img', 'iframe', 'video', 'audio', 'source', 'input', 'textarea', 'twitterwidget'];
    ...
    async save() {
      ...
      let measuringEnd;
      return Promise.resolve(extractedBlock).then(finishedExtraction => {
    ...
    compose() {
      const wrapper = Dom.make('div', Block.CSS.wrapper),
  ...
  class InlineToolbar extends Module {
    ...
    getInlineToolbarSettings(toolName) {
      ...
      if (commonInlineToolbarSettings === true) {
        const defaultToolsOrder = Object.entries(this.Editor.Tools.available).filter(([name, tool]) => {
    ...
    make() {
      this.nodes.wrapper = Dom.make('div', [this.CSS.inlineToolbar, ...(this.isRtl ? [this.Editor.UI.CSS.editorRtlFix] : [])]);
      /**
      ...
      this.Editor.Listeners.on(this.nodes.wrapper, 'mousedown', event => {
        const isClickedOnActionsWrapper = event.target.closest(`.${this.CSS.actionsWrapper}`); // If click is on actions wrapper,
    ...
    allowedToShow() {
      ...
      const selectedText = SelectionUtils.text; // old browsers
      ...
      const target = !Dom.isElement(currentSelection.anchorNode) ? currentSelection.anchorNode.parentElement : currentSelection.anchorNode;
    ...
    addTool(toolName, tool) {
      ...
      const internalTools = Object.entries(Tools.internalTools).filter(([, toolClass]) => {
        ...
        function l(t, e) {
          for (var n = 0; n < t.length; n++) {
            ...
            if (r) {
              ...
              for (a = 0; a < o.parts.length; a++) c.push(h(o.parts[a], e));
  ...
  class Paste extends Module {
    constructor() {
      ...
      this.handlePasteEvent = async event => {
        const {
          BlockManager,
          Toolbar
        } = this.Editor;
    ...
    async processDataTransfer(dataTransfer, isDragNDrop = false) {
      const {
        Sanitizer
      } = this.Editor;
      ...
      const customConfig = _extends({}, toolsTags, Sanitizer.getAllInlineToolsConfig(), {
        br: {}
      });
    ...
    async processText(data, isHTML = false) {
      const {
        Caret,
        BlockManager,
        Tools
      ...
      dataToInsert.map(async (content, i) => this.insertBlock(content, i === 0 && needToReplaceCurrentBlock));
    ...
    getTagsConfig(name, toolPasteConfig) {
      ...
      tags.forEach(tag => {
        if (Object.prototype.hasOwnProperty.call(this.toolsTags, tag)) {
          log(`Paste handler for ${name}» Tool on ${tag}» tag is skipped ` + `because it is already used by ${this.toolsTags[tag].tool}» Tool.`, 'warn');
          return;
        }
    ...
    async processFiles(items) {
      ...
      let dataToInsert;
      dataToInsert = await Promise.all(Array.from(items).map(item => this.processFile(item)));
    ...
    async processFile(file) {
      ...
      const foundConfig = Object.entries(this.toolsFiles).find(([toolName, {
        mimeTypes,
        extensions
      }]) => {
    ...
    processHTML(innerHTML) {
      ...
      return nodes.map(node => {
        ...
        const {
          tags
        } = Tools.blockTools[tool].pasteConfig;
        const toolTags = tags.reduce((result, tag) => {
          result[tag.toLowerCase()] = {};
          return result;
        }, {});

        const customConfig = _extends({}, toolTags, Sanitizer.getInlineToolsConfig(tool));
    ...
    async processSingleBlock(dataToInsert) {
      const {
        Caret,
        BlockManager,
        Tools
    ...
    async processInlinePaste(dataToInsert) {
      const {
        BlockManager,
        Caret,
        Sanitizer,
      ...
      if (currentBlockIsDefault && content.textContent.length < Paste.PATTERN_PROCESSING_MAX_LENGTH) {
        ...
        if (blockData) {
          const needToReplaceCurrentBlock = BlockManager.currentBlock && Tools.isDefault(BlockManager.currentBlock.tool) && BlockManager.currentBlock.isEmpty;
    ...
    processElementNode(node, nodes, destNode) {
      ...
      const {
        tool = ''
      } = this.toolsTags[element.tagName] || {};
      ...
      const containsAnotherToolTags = Array.from(element.children).some(({
        tagName
      }) => tags.includes(tagName) && !toolTags.includes(tagName));
      const containsBlockElements = Array.from(element.children).some(({
        tagName
      }) => Dom.blockElements.includes(tagName.toLowerCase()));
  ...
  class ReadOnly extends Module {
    ...
    async toggle(state = !this.readOnlyEnabled) {
  ...
  class RectangleSelection extends Module {
    ...
    startSelection(pageX, pageY) {
      ...
      const selectorsToAvoid = [`.${Block.CSS.content}`, `.${this.Editor.Toolbar.CSS.toolbar}`, `.${this.Editor.InlineToolbar.CSS.inlineToolbar}`];
    ...
    changingRectangle(event) {
      ...
      const {
        ...
        leftPos,
        index
      } = this.genInfoForMouseSelection(); // There is not new block in selection
    ...
    trySelectNextBlock(index) {
      ...
      const reduction = !generalSelection; // When the selection is too fast, some blocks do not have time to be noticed. Fix it.
      ...
      let i = sizeStack - 1;
      let cmp; // cmp for different directions
  ...
  class Renderer extends Module {
    ...
    async render(blocks) {
      const chainData = blocks.map(block => ({
        function: () => this.insertBlock(block)
      }));

      const _sequence = await sequence(chainData);
  ...
  // TODO: not exhaustive?
  const blockElementNames = ['P', 'LI', 'TD', 'TH', 'DIV', 'H1', 'H2', 'H3', 'H4', 'H5', 'H6', 'PRE'];

  function isBlockElement(node) {
    return blockElementNames.indexOf(node.nodeName) !== -1;
  }

  const inlineElementNames = ['A', 'B', 'STRONG', 'I', 'EM', 'SUB', 'SUP', 'U', 'STRIKE'];
  ...
  class HTMLJanitor {
    constructor(config) {
      ...
      const validConfigValues = tags.map(k => {
    ...
    _sanitize(document, parentNode) {
      ...
      do {
        ...
        const isNestedBlockElement = isBlockElement(parentNode) && isBlockElement(node) && isNotTopContainer;
        const nodeName = node.nodeName.toLowerCase();
        const allowedAttrs = getAllowedAttrs(this.config, nodeName, node);
        const isInvalid = isInline && containsBlockElement; // Drop tag entirely according to the whitelist *and* if the markup
        // is invalid.
  ...
  class Sanitizer extends Module {
    ...
    getAllInlineToolsConfig() {
      ...
      const config = {};
      Object.entries(Tools.inline).forEach(([, inlineTool]) => {
      ...
      return function (t) {
        ...
        class i {
          ...
          show(t, e, o) {
            ...
            switch (this.nodes.wrapper.classList.remove(...Object.values(this.CSS.placement)), i.placement) {
              case "top":
                this.placeTop(t, i);
                break;
            ...
            i && i.delay ? this.showingTimeout = setTimeout(() => {
  ...
  class UI extends Module {
    ...
    make() {
      ...
      this.nodes.wrapper = Dom.make('div', [this.CSS.editorWrapper, ...(this.isRtl ? [this.CSS.editorRtlFix] : [])]);
  ...
  class DragNDrop extends Module {
    ...
    enableModuleBindings() {
      ...
      this.readOnlyMutableListeners.on(UI.nodes.holder, 'drop', async dropEvent => {
  ...
  class Caret extends Module {
    ...
    set(element, offset = 0) {
      const {
        top,
        bottom
      } = SelectionUtils.setCursor(element, offset);
  ...
  class BlockSelection extends Module {
    ...
    clearSelection(reason, restoreSelection = false) {
      ...
      const _isPrintableKey = isKeyboard && isPrintableKey(reason.keyCode);
      /**
  ...
  class Blocks {
    ...
    move(toIndex, fromIndex) {
       ...
       */
      const block = this.blocks.splice(fromIndex, 1)[0]; // manipulate DOM
  ...
  class BlockManager extends Module {
    ...
    async mergeBlocks(targetBlock, blockToMerge) {
  ...
  class BlockEvents extends Module {
    ...
    backspace(event) {
      ...
      const canMergeBlocks = Caret.isAtStart && SelectionUtils.isCollapsed && currentBlock.currentInput === currentBlock.firstInput && !isFirstBlock;
    ...
    arrowRightAndDown(event) {
      const isFlipperCombination = Flipper.usedKeys.includes(event.keyCode) && (!event.shiftKey || event.keyCode === keyCodes.TAB);
      /**
   ...
   */
  const Modules = [...APIModules, ...ToolbarModules, ...RootModules];
  ...
  class Core {
    ...
    async start() {
      const modulesToPrepare = ['Tools', 'UI', 'BlockManager', 'Paste', 'BlockSelection', 'RectangleSelection', 'CrossBlockSelection', 'ReadOnly'];
